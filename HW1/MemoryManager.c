...
{
    int runningCount = 0; // inititalize counting var
    int numIterations = get_iteration_count(rand()); // number of times an arr is checked
    printf("[MemoryManager] Number of Iterations: %d \n", numIterations);
    for(int j = 0; j<numIterations;j++) //outer loop for making multiple arrays
    {
        int arr_size = get_arr_size(rand()); //get the random size of the arr
        int *arr = (int *)malloc(arr_size *sizeof(int)); //allocate mem on heap for arr
        for(int i = 0; i<arr_size;i++)   // loop for filling array with random vals
        {
            arr[i] = rand();
        }
        int median = return_median(arr,arr_size); //get median
        if(median%13 == 0) runningCount++; // increment the count if div by 13
        free(arr); //free memory used by array
